...
export function gerarPdfOrcamento(orcamento: Orcamento, cliente?: Cliente, empresa?: Empresa, oportunidade?: OportunidadeComercial) {
  ...
  autoTable(doc, {
    ...
    body: orcamento.itens.map((it, idx) => [idx + 1, toText(it.produtoNome), `${toText(it.dosePorHa)} ${toText(it.unidadeDose)}`, `${toText(it.areaHa)} ha`, toText(it.quantidadeTotal), toText(it.unidadeProduto), fmtBRL(it.precoUnitario), fmtBRL(it.valorTotalItem), fmtBRL(it.custoPorHaItem)]),
  ...
  y += 10;
  doc.setFontSize(8.5);
  doc.text(`Validade da proposta até ${toText(orcamento.validade)}. Sujeita a disponibilidade de estoque e confirmação comercial.`, margem, y);
