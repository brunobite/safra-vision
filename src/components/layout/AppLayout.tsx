  ...
  return (
    <Sidebar collapsible="icon">
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        <div className="min-w-0 rounded-md bg-sidebar-accent/40 px-2 py-2 text-sidebar-foreground group-data-[collapsible=icon]:hidden">
...
export default function AppLayout() {
  ...
  const syncBlockReason = (() => {
    ...
    if (
      syncError.includes("indisponível") ||
      syncError.includes("não autenticado")
    )
      ...
      <div className="flex min-h-screen w-full bg-background">
        ...
        <div className="flex flex-1 flex-col min-w-0">
          <header className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b border-border bg-card/80 px-3 backdrop-blur md:gap-3 md:px-4">
            ...
            <div className="flex min-w-0 flex-col leading-tight">
              <h1 className="truncate text-sm font-semibold text-foreground">
                <span className="md:hidden">Safra 26/27</span>
                <span className="hidden md:inline">
                  Safra 26/27 — Controle Operacional
                </span>
              </h1>
              <span className="hidden text-[11px] text-muted-foreground sm:inline">
                Gestão comercial agrícola
              </span>
            </div>
            <div className="ml-auto min-w-0 text-right text-[10px] leading-tight md:text-[11px]">
              ...
              {syncText && (
                <div
                  className={
                    syncStatus === "error"
                      ? "max-w-[9rem] truncate text-destructive md:max-w-xs"
                      : pendingSyncCount > 0
                        ? "max-w-[9rem] truncate text-amber-700 md:max-w-xs"
              ...
              {syncStatus === "error" && syncError && (
                <div
                  className="hidden max-w-xs truncate text-destructive/80 sm:block"
                  title={syncError}
                >
                  {syncError}
                </div>
              ...
              <div className="hidden text-muted-foreground sm:block">
                Perfil: {role}
                {vendedorNome ? ` • ${vendedorNome}` : ""} • {accessStatus}
              </div>
            </div>
          </header>
          ...
          <div className="fixed bottom-5 right-5 z-40 hidden md:block">
            ...
            {open && (
              <div className="absolute bottom-14 right-0 w-56 rounded border bg-card p-2 text-sm shadow">
            ...
            <div className="mx-auto grid max-w-md grid-cols-2 gap-3">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    className="h-11 min-w-11 rounded-xl shadow-sm"
                    aria-label="Abrir ações rápidas"
                  >
                ...
                <DropdownMenuContent
                  ...
                  side="top"
                  sideOffset={12}
                  className="z-40 mb-1 w-[min(22rem,calc(100vw-2rem))] p-2"
                >
