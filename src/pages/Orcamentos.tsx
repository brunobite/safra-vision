...
const STATUS_NOVO: OrcamentoStatus[] = ["Rascunho"];
...
const SEGMENTOS_BASE = ["Adjuvante", "Nutrição", "Fertilizante", "Sementes", "Defensivos", "Biológicos", "Consultoria", "Outros"];
...
const formasPagamentoOptions = formasPagamentoAtivas.length > 0 ? formasPagamentoAtivas : FORMAS_PAGAMENTO_PADRAO;
const proximasAcoesDisponiveis = proximasAcoes.filter((a) => {
  if (a.status !== "Pendente") return false;
  if (!form.clienteId) return false;
  if (a.clienteId !== form.clienteId) return false;
  if (form.oportunidadeId) return !a.oportunidadeId || a.oportunidadeId === form.oportunidadeId;
  return !a.oportunidadeId;
});

useEffect(() => { const oportunidadeId = params.get("oportunidadeId"); if (oportunidadeId) { const op = oportunidades.find(x => x.id === oportunidadeId); if (op) { const itens = (op.itensEstimados || []).map((it, idx) => ({ id: `i${Date.now()}-${idx}`, produtoId: it.produtoId, produtoNome: it.produtoNome || "", categoria: it.categoria || "", unidadeProduto: it.unidadeProduto || "LT", dosePorHa: it.dosePorHa || 0, unidadeDose: it.unidadeDose || "L/ha", areaHa: it.areaHa || 0, quantidadeTotal: it.quantidadeTotal || 0, precoUnitario: it.precoUnitario || 0, valorTotalItem: it.valorTotalItem || 0, custoPorHaItem: it.custoPorHaItem || 0, observacoes: it.observacoes })); const areaAplicacao = itens.length ? Math.max(...itens.map((i) => i.areaHa || 0), 0) : 0; setEdit(null); setForm(f => recalc({ ...f, clienteId: op.clienteId, oportunidadeId: op.id, responsavel: op.responsavel || f.responsavel || "", vendedor: op.responsavel || f.vendedor || "", segmento: op.segmento || f.segmento || "", areaAplicacaoHa: areaAplicacao || f.areaAplicacaoHa || 0, itens })); setOpen(true); return; } } const negocioId = params.get("negocioId"); if (!negocioId) return; const n = negocios.find(x => x.id === negocioId); if (!n) return; const ja = orcamentos.find(o => o.negocioId===negocioId); if (ja) { setEdit(ja); setForm(ja); setOpen(true); return; } setForm(f=>({ ...f, clienteId: n.clienteId, negocioId: n.id, vendedor: n.vendedor })); setOpen(true); }, [params, oportunidades, negocios, orcamentos]);
...
const save = () => { const payload = recalc({ ...form, updatedAt: new Date().toISOString() }); if (!payload.clienteId) return toast.error("Cliente obrigatório"); if (!isLegacy && !payload.oportunidadeId) return toast.error("Oportunidade obrigatória para novo orçamento"); setOrcamentos(p => edit ? p.map(o => o.id === edit.id ? payload : o) : [{ ...payload, id: `orc${Date.now()}`, createdAt: new Date().toISOString() }, ...p]); if (payload.status === "Aprovado") { const valor = payload.valorTotal || 0; setNegocios(prev => { const existente = payload.negocioId ? prev.find(n => n.id === payload.negocioId) : prev.find(n => n.clienteId === payload.clienteId && n.status === "Fechado ganho" && Math.abs((n.valorFechado || 0) - valor) < 0.01); if (existente) return prev.map(n => n.id === existente.id ? { ...n, status: "Fechado ganho", valorFechado: n.valorFechado || valor, ultimaAtualizacao: new Date().toISOString().slice(0,10) } : n); return prev; }); } if ((payload.status === "Perdido" || payload.status === "Recusado" || payload.status === "Vencido") && payload.data) { setProximasAcoes(prev => [{ id: `pa${Date.now()}`, clienteId: payload.clienteId, orcamentoId: payload.id, responsavel: payload.responsavel || payload.vendedor || "", descricao: `Follow-up pós orçamento perdido: ${payload.codigo}`, tipo: "Ligação", data: payload.data, status: "Pendente", origem: "Orçamento", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }, ...prev]); } setOpen(false); toast.success("Orçamento salvo"); };
useEffect(() => {
...
const novoOrcamento = () => {
  setEdit(null);
  setForm({ id: "", codigo: `ORC-${Date.now()}`, clienteId: "", empresaId: empresaPadrao, vendedor: "", data: new Date().toISOString().slice(0, 10), validade: validade7(new Date().toISOString().slice(0, 10)), status: "Rascunho", areaAplicacaoHa: 0, itens: [], subtotal: 0, descontoTotal: 0, valorTotal: 0, custoPorHectare: 0, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), prazoPagamento: "", formaPagamento: formaPagamentoPadrao });
  setOpen(true);
};
const criarNovaAcao = () => {
  if (!form.clienteId || !form.oportunidadeId) return toast.error("Selecione cliente e oportunidade para criar follow-up");
  const id = `pa${Date.now()}`;
  setProximasAcoes((prev) => [{
    id,
    clienteId: form.clienteId,
    oportunidadeId: form.oportunidadeId,
    responsavel: novaAcao.responsavel || form.responsavel || form.vendedor || "",
    descricao: novaAcao.descricao || "Follow-up do orçamento",
    objetivo: novaAcao.objetivo || undefined,
    tipo: "Follow-up",
    data: novaAcao.data,
    status: "Pendente",
    origem: "Orçamento",
  ...
  setForm((prev) => ({ ...prev, proximaAcaoId: id }));
  setShowNovaAcao(false);
  toast.success("Follow-up criado e vinculado");
};
...
<div><Label>Segmento comercial</Label><Select value={form.segmento || ""} onValueChange={v=>setForm({...form, segmento:v})}><SelectTrigger><SelectValue placeholder="Selecione"/></SelectTrigger><SelectContent>{segmentoOptions.map((s)=><SelectItem key={s} value={s}>{s}</SelectItem>)}</SelectContent></Select></div>
<div><Label>Responsável</Label><Input value={form.responsavel || form.vendedor || ""} onChange={e=>setForm({...form,responsavel:e.target.value, vendedor:e.target.value})} /></div><div><Label>Cliente</Label><Select value={form.clienteId} onValueChange={v=>setForm({...form, clienteId:v, oportunidadeId: undefined, proximaAcaoId: undefined})}><SelectTrigger><SelectValue/></SelectTrigger><SelectContent>{clientes.map(c=><SelectItem key={c.id} value={c.id}>{c.nome}</SelectItem>)}</SelectContent></Select></div><div><Label>Oportunidade vinculada</Label><Select value={form.oportunidadeId || (isLegacy ? "legacy" : "")} onValueChange={v=>setForm({...form, oportunidadeId:v === "legacy" ? undefined : v, proximaAcaoId: undefined})}><SelectTrigger><SelectValue placeholder={isLegacy ? "Compatibilidade legado" : "Obrigatório para novo orçamento"}/></SelectTrigger><SelectContent>{isLegacy && <SelectItem value="legacy">Orçamento legado/sem vínculo</SelectItem>}{oportunidadesAbertasCliente.map(o=><SelectItem key={o.id} value={o.id}>{o.etapa} · {o.necessidade || o.id}</SelectItem>)}</SelectContent></Select>{form.clienteId && oportunidadesAbertasCliente.length === 0 && !form.oportunidadeId && <p className="text-xs text-amber-600 mt-1">Este cliente ainda não possui oportunidade aberta. Crie uma oportunidade antes de salvar.</p>}</div>
...
<div><Label>Próxima ação</Label><Select value={form.proximaAcaoId || "none"} onValueChange={v=>{ if (v === "new") { setShowNovaAcao(true); return; } setForm({...form,proximaAcaoId:v === "none" ? undefined : v}); }}><SelectTrigger><SelectValue placeholder="Opcional"/></SelectTrigger><SelectContent><SelectItem value="none">Sem vínculo</SelectItem><SelectItem value="new">Criar nova ação de follow-up</SelectItem>{proximasAcoesDisponiveis.map(a=><SelectItem key={a.id} value={a.id}>{a.descricao}</SelectItem>)}</SelectContent></Select></div>
...
<div><Label>Forma de pagamento</Label><Select value={form.formaPagamento || ""} onValueChange={v=>setForm({...form,formaPagamento:v})}><SelectTrigger><SelectValue placeholder="Selecione"/></SelectTrigger><SelectContent>{formasPagamentoOptions.map((fp)=><SelectItem key={fp} value={fp}>{fp}</SelectItem>)}</SelectContent></Select></div></div>
...
<Dialog open={showNovaAcao} onOpenChange={setShowNovaAcao}>
  <DialogContent>
    <DialogHeader><DialogTitle>Nova ação de follow-up</DialogTitle></DialogHeader>
    <div className="grid gap-2">
      <div><Label>Data</Label><Input type="date" value={novaAcao.data} onChange={(e)=>setNovaAcao((p)=>({ ...p, data: e.target.value }))} /></div>
      <div><Label>Responsável</Label><Input value={novaAcao.responsavel} onChange={(e)=>setNovaAcao((p)=>({ ...p, responsavel: e.target.value }))} /></div>
      <div><Label>Descrição</Label><Input value={novaAcao.descricao} onChange={(e)=>setNovaAcao((p)=>({ ...p, descricao: e.target.value }))} /></div>
      <div><Label>Objetivo</Label><Input value={novaAcao.objetivo} onChange={(e)=>setNovaAcao((p)=>({ ...p, objetivo: e.target.value }))} /></div>
    </div>
    <DialogFooter><Button variant="outline" onClick={()=>setShowNovaAcao(false)}>Cancelar</Button><Button onClick={criarNovaAcao}>Criar follow-up</Button></DialogFooter>
  </DialogContent>
</Dialog></div>;
}
