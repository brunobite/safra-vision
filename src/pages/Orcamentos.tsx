...
const STATUS: OrcamentoStatus[] = ["Aberto", "Enviado", "Em negociação", "Aprovado", "Recusado", "Vencido", "Rascunho", "Reprovado", "Cancelado"];
const validade7 = (base: string) => new Date(new Date(base).getTime() + 7 * 86400000).toISOString().slice(0, 10);

export default function Orcamentos() {
const { orcamentos, setOrcamentos, clientes, produtos, empresas, negocios, formasPagamento, proximasAcoes, setNegocios, setProximasAcoes } = useAppStore();
...
<div><Label>Prev. fechamento</Label><Input type="date" value={form.dataPrevistaFechamento || ""} onChange={e=>setForm({...form,dataPrevistaFechamento:e.target.value})} /></div>
<div><Label>Data aprovação/perda</Label><Input type="date" value={form.dataDecisao || ""} onChange={e=>setForm({...form,dataDecisao:e.target.value})} /></div>
...
<div className="md:col-span-2"><Label>Motivo da perda</Label><Input value={form.motivoPerda || ""} onChange={e=>setForm({...form,motivoPerda:e.target.value})} /></div>
