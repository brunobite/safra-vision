...
useEffect(() => { const oportunidadeId = params.get("oportunidadeId"); if (oportunidadeId) { const op = oportunidades.find(x => x.id === oportunidadeId); if (op) { setEdit(null); setForm(f => ({ ...f, clienteId: op.clienteId, oportunidadeId: op.id, responsavel: op.responsavel || f.responsavel || "", vendedor: op.responsavel || f.vendedor || "", segmento: op.segmento || f.segmento || "" })); setOpen(true); return; } } const negocioId = params.get("negocioId"); if (!negocioId) return; const n = negocios.find(x => x.id === negocioId); if (!n) return; const ja = orcamentos.find(o => o.negocioId===negocioId); if (ja) { setEdit(ja); setForm(ja); setOpen(true); return; } setForm(f=>({ ...f, clienteId: n.clienteId, negocioId: n.id, vendedor: n.vendedor })); setOpen(true); }, [params, oportunidades, negocios, orcamentos]);
