...
export default function Lancamentos() {
  ...
  const salvar = (gerarOrcamento = false) => {
    ...
    if (form.geraOportunidade) {
      ...
      const valorPotencial = itensCalculados.reduce((s, i) => s + i.valorTotalItem, 0);
      setNegocios(prev => [{ id: negId, nome: form.oppNome || `Oportunidade ${clienteById(form.clienteId)?.nome}`, clienteId: form.clienteId, vendedor: form.vendedor || "Bruno", origem: "Visita" as OrigemNegocio, produtos: form.oppProdutos.map(p => p.produtoId).filter(Boolean), categoria: form.oppCategoria, valorPotencial, status: form.oppStatus, previsaoFechamento: form.oppDataProxAcao, dataCriacao: hoje, ultimaAtualizacao: hoje, proximaAcao: form.proximaAcao || "", dataProximaAcao: form.dataProximaAcao || "", observacoes: [form.oppCultura && `Cultura: ${form.oppCultura}`, form.oppObservacoes].filter(Boolean).join(" | "), lancamentoId: id }, ...prev.filter(n => n.id !== negId)]);
      negocioId = negId;
    }
    ...
    if (form.proximaAcao && form.dataProximaAcao) setProximasAcoes(prev => [{ id: `pa${Date.now()}`, clienteId: form.clienteId, negocioId, responsavel: form.vendedor, descricao: form.proximaAcao!, tipo: form.tipoProximaAcao || "Visita", data: form.dataProximaAcao!, status: "Pendente", origem: "Lançamento", createdAt: agora.toISOString(), updatedAt: agora.toISOString() }, ...prev]);
    ...
    if (gerarOrcamento && negocioId) {
      ...
      setOrcamentos(prev => [{ id: `orc${Date.now()}`, codigo: `ORC-${Date.now()}`, clienteId: form.clienteId, negocioId, empresaId: empresaPadrao, vendedor: form.vendedor || "", data: hoje, validade: new Date(agora.getTime() + 7 * 86400000).toISOString().slice(0, 10), status: "Rascunho", areaAplicacaoHa: form.oppAreaAplicacaoHa, itens: itensCalculados, subtotal, descontoTotal: 0, valorTotal: subtotal, custoPorHectare: form.oppAreaAplicacaoHa > 0 ? subtotal / form.oppAreaAplicacaoHa : 0, createdAt: agora.toISOString(), updatedAt: agora.toISOString() }, ...prev]);
      nav(`/orcamentos?negocioId=${negocioId}`);
    }
